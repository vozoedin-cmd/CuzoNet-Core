...
export class ListNetworkLinksUseCase {
  constructor(private readonly repository: NetworkLinkRepository) {}

  public async execute(companyId: string): Promise<NetworkLinkDto[]> {
    const links = await this.repository.listByCompanyId(companyId);
