...
export class SqliteEquipmentRepository implements EquipmentRepository {
  ...
  public async findById(id: string): Promise<Equipment | null> {
    const row = this.db.prepare('SELECT * FROM network_assets WHERE id = ?').get(id) as Record<
      string,
      unknown
    >;
    if (!row) return null;
    ...
    const interfaces = this.db
      .prepare('SELECT * FROM asset_interfaces WHERE asset_id = ?')
      .all(id) as Record<string, unknown>[];
    const assignments = this.db
      .prepare('SELECT * FROM asset_assignments WHERE asset_id = ?')
      .all(id) as Record<string, unknown>[];
    ...
    return Equipment.create({
      ...
      ...(row.management_host === null
        ? {}
        : { managementHost: ManagementHost.create(row.management_host as string) }),
    });
  }
  ...
  public async save(equipment: Equipment): Promise<void> {
    ...
    this.db
      .prepare(
        `
      INSERT INTO network_assets (id, company_id, asset_model_id, serial_number, mac_address, management_host, asset_type, status, acquired_on, role, capabilities)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
