...
export const environmentSchema = z.object({
  ...
  PROVISIONING_WORKER_ENABLED: booleanStringSchema.default(false),
  PROVISIONING_WORKER_INTERVAL_MS: z.coerce.number().int().min(100).max(60000).default(5000),
  PROVISIONING_WORKER_BATCH_SIZE: z.coerce.number().int().min(1).max(200).default(20),
  PROVISIONING_WORKER_LEASE_SECONDS: z.coerce.number().int().min(5).max(3600).default(60),
  PROVISIONING_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(10).default(5),
  ROUTEROS_PROVISIONING_ENABLED: booleanStringSchema.default(false),
  ROUTEROS_CONNECT_TIMEOUT_MS: z.coerce.number().int().min(100).max(60_000).default(5_000),
  ROUTEROS_COMMAND_TIMEOUT_MS: z.coerce.number().int().min(100).max(60_000).default(10_000),
  ROUTEROS_MAX_CONNECTIONS: z.coerce.number().int().min(1).max(100).default(5),
  ROUTEROS_TLS_VERIFY: booleanStringSchema.default(true),
...
const parsedEnvironment = environmentSchema.safeParse({
  ...
  PROVISIONING_WORKER_ENABLED: process.env.PROVISIONING_WORKER_ENABLED,
  PROVISIONING_WORKER_INTERVAL_MS: process.env.PROVISIONING_WORKER_INTERVAL_MS,
  PROVISIONING_WORKER_BATCH_SIZE: process.env.PROVISIONING_WORKER_BATCH_SIZE,
  PROVISIONING_WORKER_LEASE_SECONDS: process.env.PROVISIONING_WORKER_LEASE_SECONDS,
  PROVISIONING_MAX_ATTEMPTS: process.env.PROVISIONING_MAX_ATTEMPTS,
  ROUTEROS_PROVISIONING_ENABLED: process.env.ROUTEROS_PROVISIONING_ENABLED,
  ROUTEROS_CONNECT_TIMEOUT_MS: process.env.ROUTEROS_CONNECT_TIMEOUT_MS,
  ROUTEROS_COMMAND_TIMEOUT_MS: process.env.ROUTEROS_COMMAND_TIMEOUT_MS,
  ROUTEROS_MAX_CONNECTIONS: process.env.ROUTEROS_MAX_CONNECTIONS,
  ROUTEROS_TLS_VERIFY: process.env.ROUTEROS_TLS_VERIFY,
