...
/**
 * Stable business identifier for a RouterOS firewall NAT rule. Embedded in
 * the rule's comment marker (see NatRuleComment) so identity never depends
 * on RouterOS's mutable ".id", which can change after exports, restores or
 * recreations.
 */
export class NatRuleReference {
  private constructor(public readonly value: string) {}

  public static create(raw: string): NatRuleReference {
    ...
    return new NatRuleReference(value);
  }
}
